...
aveg <- median(as.numeric(df$numsenior))
aveg
#Column added to DataFrame depicting whether the number of seniors is higher than the median number of seniors
newdf <- df %>%
  mutate(Age = ifelse(as.numeric(numsenior) < as.numeric(aveg), "Young",
                 ifelse(as.numeric(numsenior) > as.numeric(aveg), "Old", NA))) 
...
nrow(newdf)
#chi2 test
test <- chisq.test(as.numeric(tab))
test$expected
