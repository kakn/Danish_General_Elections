...
two_thirds_over_mean_qualification <- function(P,pr,vd) # P = party of interest, pr = provinces, vd = vote distribution data frame
{
  ...
  for (i in 1:length(pr))
  {
    ...
    mean_votes_per_seat = province_votes / province_consituency_seats[i] # Calculates mean votes per constituency seat in province
...
```{r}
qualified_parties
for (i in 1:length(qualified_parties))
{
print(sum(subset(vote_distribution, Party == qualified_parties[i])$Votes) / sum(subset(vote_distribution,Party %in% qualified_parties)$Votes)*175)
}
```
